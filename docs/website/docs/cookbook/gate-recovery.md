...
After upgrading the ctx binary, check whether the installed integration still matches the embedded
templates, then regenerate the target you actually use:

```bash
ctx harness doctor --json
ctx harness init --target <claude|codex>
ctx harness doctor --json
```

Upgrading the executable does not update project-local hooks. A normal `harness init` safely
regenerates files that still match their recorded checksum and warns instead of overwriting files
you modified; `.ctx/rules.toml` is never overwritten. Review every skipped file rather than reaching
for `--force`, because a local customization may be intentional. In the ctx-on-ctx trial, the 0.3.5
binary correctly identified hook templates generated by 0.3.0 as stale.
...
| `ctx harness init` after a binary upgrade | Regenerates unmodified hooks from the new binary templates | Modified files are skipped for review; policy rules are preserved |
...
not delete rules, raise thresholds blindly, force-overwrite modified harness files, or turn
operational failure into success. After a ctx binary upgrade, run harness doctor, regenerate the
configured harness target without --force, inspect skipped files, and run doctor again.
```
