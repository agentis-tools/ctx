...
oversized-top behavior. A zero budget is rejected. `--dry-run` intentionally
previews all relevant files because it does not emit a context document; use
`--count-only` to inspect the budgeted selection.
...
- [Embeddings](../code-intelligence.md#embeddings) - Generate embeddings
- [Call Graph](../code-intelligence.md#call-graph) - Manual call graph queries
